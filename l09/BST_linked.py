...
class BST:
    ...
    def _eq_aux(self, source_node, target_node):
        if source_node is None and target_node is None:
            equals=True
        elif source_node is not None and target_node is not None and source_node._value == target_node._value:
            left_equal = self._eq_aux(source_node._left, target_node._left)
            right_equal = self._eq_aux(source_node._right, target_node._right)
            equals=left_equal and right_equal
        else:
            equals=False
    ...
    def balanced_aux(self, node):
        if node is None or node._height == 1:
            ...
            balanced = self.balanced_aux(node._left) and self.balanced_aux(node._right)
        return balanced
