...
class List:
    ...
    def reverse_r_aux(self, prev, current):
        """
        -------------------------------------------------------
        Reverses the order of the elements in list.
        (recursive algorithm)
    ...
    def _linear_search_r(self, key):
        ...
        index=0
        if self._front is not None:
            prev, curr, index = self._linear_search_r_aux(key, prev, curr, index)
        else:
            index=-1
        return prev, curr, index


    def _linear_search_r_aux(self, key, prev, curr, index):
        """
        -------------------------------------------------------
        Searches for the first occurrence of key in the list.
        Private helper methods - used only by other ADT methods.
        ...
        if curr is None:
            ...
            prev, curr, index = self._linear_search_r_aux(key, curr, curr._next, index+1)
        else:
            index=-1
        return prev, curr, index
    ...
    def intersection_r_aux(self, source2, node1):
        """
        -------------------------------------------------------
        Update the current list with values that appear in both
        source1 and source2. Values do not repeat. source1 and
        ...
        if node1 is not None:
            ...
            if current is not None:
                ...
                if current is None:
                    self.append(value)
            self.intersection_r_aux(source2, node1._next)
